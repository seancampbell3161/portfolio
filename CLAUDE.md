...
- `npm run e2e` — Playwright pass over the home timeline's scrub and pan interactions at desktop and phone widths (hover, pin, drag, tap, keys, deep links), plus the hero readout's links and its pruning under a fixed clock past the build day. Needs `npm run preview` running.
