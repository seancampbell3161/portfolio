...
- `npm run e2e` — Playwright pass over the home timeline's scrub and pan interactions at desktop and phone widths (hover, pin, drag, tap, keys, deep links), the hero readout's links and its pruning under a fixed clock past the build day, and the reader frame's reading line and sticky sidebar on an essay, a short case study and a phone. Needs `npm run preview` running.
