...
- **`src/layouts/ProjectPage.astro`** (new): the project content of §4.3 inside `Reader`. Props: the collection entry, `segment: TrackRow[]`, and `alongside: TimelineItem[]` (the `during` query's result, named so it does not shadow the imported function).
