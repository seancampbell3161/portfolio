...
- **Controls during a wake.** "Next wake" carries `aria-disabled="true"` while a wake runs and clicks are ignored, so focus stays where it is. Pressed during Play's gap, it starts the next wake at once and Play continues. A mechanism switch presses its button, sets the root's `data-mechanism` and, when no wake is running, paints the idle frame for the new mechanism, clearing the still frame or a held reduced-motion frame; pressing the already-pressed mechanism does nothing; a running wake keeps the mechanism it started with. A count switch cancels the running wake, paints idle, rebuilds the grid by cloning the first cell, sets `--cols` and `data-numbered`, resets the tallies and their rows, writes `resetText`, and stops Play.
