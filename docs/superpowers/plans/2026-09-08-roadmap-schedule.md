...
```astro
  <div class="rm-week-lists">
    <PairingList label="Reading" items={w?.reading ?? []} />
    <PairingList label="Foundations" items={w?.foundations ?? []} />
  </div>
```

Then update `src/scripts/roadmap-schedule.ts`: since the lists are now server-rendered per phase and a phase changes only every few weeks, the script keeps rewriting only the label and phase name. Delete the now-unused `[data-week-reading]` / `[data-week-foundations]` hooks from the contract test added in Task 4, replacing that assertion with:

```ts
  it("keeps the hooks the band's script writes into", () => {
    for (const hook of ["data-week-label", "data-week-phase"]) {
      expect(html, `missing ${hook}`).toContain(hook);
    }
  });
```
...
Run: `npm run check`
Expected: PASS, including the Redis-panel assertion.

- [ ] **Step 7: Commit**

```bash
git add src/components/roadmap/PairingList.astro src/components/roadmap/RoadmapInspector.astro src/components/roadmap/ThisWeek.astro src/scripts/roadmap-schedule.ts src/__tests__/roadmap-contract.test.ts
git commit -m "feat(roadmap): each clip names what to read alongside it, and why"
```
